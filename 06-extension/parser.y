...
%token IF WHILE FOR RETURN INT FLOAT CHAR VOID LPAREN LCURL RCURL LTHIRD RTHIRD SEMICOLON COMMA PRINTLN PRINT SCANF
			...
			else if(symbol_info == NULL_SYMBOL_INFO)
			{
				if(!is_global_scope)
				{
					if(var_info->get_array_size() > 0)
					{
						...
						symbol_table.get_current_scope()->inc_current_offset(2);
					}
...
type_specifier	: 
	...
	CHAR 
	{
		$$ = (new SymbolInfo())->set_data_type(DataType::CHAR);
	}
	|
	VOID
	{
...
statement : 
	...
	PRINT LPAREN variable RPAREN SEMICOLON
	{
		...
		if($3->is_global())
		{
			if($3->get_array_size() < 0)
			{
				gen_mov("AX", $3->get_lexeme(), get_lineno_comment(@2.first_line));
			}
			else 
			{
				gen_pop("SI", get_lineno_comment(@2.first_line));
				gen_mov("AX", $3->get_lexeme() + "[SI]", get_lineno_comment(@2.first_line));
			}
		}
		else 
		{
			gen_pop("SI", get_lineno_comment(@1.first_line));
			gen_mov("AX", "[BP+SI]", get_lineno_comment(@1.first_line));
		}
		if($3->get_data_type()==DataType::INT)
		{
			gen_call("PRINT_INT", get_lineno_comment(@1.first_line));
		}
		else if($3->get_data_type() == DataType::CHAR)
		{
			gen_mov("DL", "AL", get_lineno_comment(@1.first_line));
			gen_mov("AH", "2", get_lineno_comment(@1.first_line));
			gen_code("INT 21H", get_lineno_comment(@1.first_line));
		}
	}
	|
	PRINTLN LPAREN variable RPAREN SEMICOLON
	{
		...
		gen_call("PRINT_NEWLINE", get_lineno_comment(@1.first_line));
	}
	|
	...
	|
	SCANF LPAREN variable RPAREN SEMICOLON
	{
		if($3->get_data_type() == DataType::INT)
		{
			gen_call("SCANLN_INT", get_lineno_comment(@1.first_line));
			gen_mov("AX", "DX", get_lineno_comment(@1.first_line));
		}
		else if($3->get_data_type() == DataType::CHAR)
		{
			gen_mov("AH", "1", get_lineno_comment(@1.first_line));
			gen_code("INT 21H", get_lineno_comment(@1.first_line));
			gen_xor("AH", "AH", get_lineno_comment(@1.first_line));
		}
		if($3->is_global())
		{
			if($3->get_array_size() < 0)
			{
				gen_mov($3->get_lexeme(), "AX", get_lineno_comment(@2.first_line));
			}
			else 
			{
				gen_pop("SI", get_lineno_comment(@2.first_line));
				gen_mov($3->get_lexeme() + "[SI]", "AX", get_lineno_comment(@2.first_line));
			}
		}
		else 
		{
...
expression : 
	...
	variable ASSIGNOP logic_expression 	
	{
		...
		else if($3->get_data_type() != $1->get_data_type())
		{
			error_default(@2.first_line, "assignment of incompatible types");
		}
		else 
		{
			if($3->has_jump())
...
factor	: 
	...
	CONST_CHAR
	{
		$1->set_data_type(DataType::CHAR);
		$$ = $1;
		gen_mov("AX", $1->get_lexeme(), get_lineno_comment(@1.first_line));
		gen_push("AX", get_lineno_comment(@1.first_line));
	}
	|
	variable INCOP 
